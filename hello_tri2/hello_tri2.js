...
Promise.all(initComplete).then((results) => {
    ...
    const shaderModule = device.createShaderModule({code: shaders["triangle_rot.wgsl"]});
    ...
    let angleBuffer = device.createBuffer({label: "angleBuffer", size: 4, usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST });
